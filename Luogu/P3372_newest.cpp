...
inline void update(const int& id, const int& l, const int& r, const long long& val) {
    if(l <= lson(id) && r >= rson(id)) { // Competely covered.
    ...
    pushDown(id);
    int mid = mid(lson(id), rson(id));
    if(l <= mid) update(lson(id), l, r, val);
    if(r > mid) update(rson(id), l, r, val);
